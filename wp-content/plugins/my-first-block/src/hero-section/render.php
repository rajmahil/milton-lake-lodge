<?php
/**
 * Hero Section Block - Fixed for responsive background images
 */
...
if ($image) {
    if (is_array($image)) {
        $desktop_image_url = $image['url'] ?? ($image['sizes']['large']['url'] ?? '');
        $desktop_image_alt = $image['alt'] ?? '';
        $desktop_image_id = $image['id'] ?? null;
    } elseif (is_numeric($image)) {
        $desktop_image_id = $image;
        $desktop_image_url = wp_get_attachment_image_url($image_id, 'large');
        $desktop_image_alt = get_post_meta($image_id, '_wp_attachment_image_alt', true);
    } elseif (is_string($image)) {
        $desktop_image_url = $image;
    }
} else {
    $desktop_image_url = get_template_directory_uri() . '/assets/images/default-hero.jpg';
    $desktop_image_alt = 'Default Hero Image';
}

// Mobile image handling - Fixed variable scope
$mobile_image_url = '';
if ($mobileImage) {
    if (is_array($mobileImage)) {
        $mobile_image_url = $mobileImage['url'] ?? ($mobileImage['sizes']['large']['url'] ?? '');
    } elseif (is_string($mobileImage)) {
        $mobile_image_url = $mobileImage;
    }
}

// Fallback: use desktop image if no mobile image provided
if (empty($mobile_image_url)) {
    $mobile_image_url = $desktop_image_url;
}

// Generate unique ID for this hero section
...
>
  <section
    class="<?php echo $unique_id; ?> h-[95vh] min-h-[800px] flex items-end overflow-hidden relative not-prose section-padding pb-6 w-full !rounded-b-3xl"
    <?php if ($desktop_image_id): ?>
    data-image-id="<?php echo esc_attr($desktop_image_id); ?>"
    <?php endif; ?>
  >
    <div
    ...
    <!-- Gradient Overlay -->
    <div class="absolute bottom-0 left-0 w-full h-full bg-gradient-to-t from-black/70 to-black/0"></div>
...
<script type="application/ld+json">
{
    ...
    "image": "<?php echo esc_js($desktop_image_url ?: ''); ?>"
}
</script>
<?php endif; ?>
