...
class Plugin
{
    ...
    {
        add_submenu_page(is_multisite() ? 'settings.php' : 'options-general.php', __('Redis Object Cache', 'redis-cache'), __('Redis', 'redis-cache'), $this->manage_redis_capability(), 'redis-cache', [$this, 'show_admin_page']);
    }
    ...
    public function show_admin_page()
    {
        ...
        UI::register_tab('metrics', __('Metrics', 'redis-cache'), ['disabled' => !Metrics::is_enabled()]);
    ...
    public function setup_dashboard_widget()
    {
        ...
        wp_add_dashboard_widget('dashboard_rediscache', __('Redis Object Cache', 'redis-cache'), [$this, 'show_dashboard_widget'], null, null, 'normal', 'high');
    }
    ...
    {
        $settings = sprintf('<a href="%s">%s</a>', network_admin_url($this->page), esc_html__('Settings', 'redis-cache'));
    ...
    public function add_plugin_row_meta(array $plugin_meta, $plugin_file)
    {
        ...
        $plugin_meta[] = sprintf('<a href="%1$s"><span class="dashicons dashicons-star-filled" aria-hidden="true" style="font-size: 14px; line-height: 1.3"></span>%2$s</a>', $this->link_to_ocp('meta-row'), esc_html_x('Upgrade to Pro', 'verb', 'redis-cache'));
    ...
    public function enqueue_admin_styles()
    {
        ...
        $screens = [$this->screen, 'dashboard', 'dashboard-network'];

        if (!in_array($screen->id, $screens, true)) {
            return;
        }

        wp_enqueue_style('redis-cache', trailingslashit(WP_REDIS_PLUGIN_DIR) . 'assets/css/admin.css', [], WP_REDIS_VERSION);
    }
    ...
    public function enqueue_admin_scripts()
    {
        ...
        $screens = [$this->screen, 'dashboard', 'dashboard-network', 'edit-shop_order', 'edit-product', 'woocommerce_page_wc-admin'];
        ...
        wp_enqueue_script('redis-cache', plugins_url('assets/js/admin.js', WP_REDIS_FILE), array_merge(['jquery', 'underscore'], $clipboard ? ['clipboard'] : []), WP_REDIS_VERSION, true);

        wp_localize_script('redis-cache', 'rediscache', [
            'jQuery' => 'jQuery',
            'disable_pro' => $screen->id !== $this->screen || (defined('WP_REDIS_DISABLE_BANNERS') && WP_REDIS_DISABLE_BANNERS) || self::acceleratewp_install(),
            'l10n' => [
    ...
    public function enqueue_redis_metrics()
    {
        ...
        wp_enqueue_script('redis-cache-charts', plugins_url('assets/js/apexcharts.min.js', WP_REDIS_FILE), [], WP_REDIS_VERSION, true);

        if (!$this->get_redis_status()) {
            return;
        }

        $min_time = $screen->id === $this->screen ? Metrics::max_time() : MINUTE_IN_SECONDS * 30;
    ...
    public function register_qm_output($output)
    {
        ...
        if ($collector instanceof QM_Collector && method_exists('QM_Output_Html', 'before_non_tabular_output')) {
    ...
    public function validate_object_cache_dropin()
    {
         ...
         */
        return apply_filters('redis_cache_validate_dropin', $dropin['PluginURI'] === $plugin['PluginURI'], $dropin['PluginURI'], $plugin['PluginURI']);
    }
    ...
    public function render_admin_bar($wp_admin_bar)
    {
        ...
        if ($redis_status) {
            ...
            $value = sprintf('%s%%&nbsp;&nbsp;%s/%s&nbsp;&nbsp;%s', $info->ratio, $hits, $misses, $size);
        ...
        if (isset($_GET['_wpnonce'], $_GET['action'])) {
            ...
            if (in_array($action, $this->actions, true)) {
                ...
                if ($action === 'flush-cache') {
                    wp_cache_flush() ? add_settings_error('redis-cache', 'flush', __('Object cache flushed.', 'redis-cache'), 'updated') : add_settings_error('redis-cache', 'flush', __('Object cache could not be flushed.', 'redis-cache'), 'error');
                }

                // do we have filesystem credentials?
                if ($this->initialize_filesystem($url, true)) {
                    if ($action === 'enable-cache') {
                        $result = $wp_filesystem->copy(WP_REDIS_PLUGIN_PATH . '/includes/object-cache.php', WP_CONTENT_DIR . '/object-cache.php', true, FS_CHMOD_FILE);
                        ...
                        $result ? add_settings_error('redis-cache', 'dropin', __('Object cache enabled.', 'redis-cache'), 'updated') : add_settings_error('redis-cache', 'dropin', __('Object cache could not be enabled.', 'redis-cache'), 'error');
                    }
                    ...
                    if ($action === 'disable-cache') {
                        ...
                        $result ? add_settings_error('redis-cache', 'dropin', __('Object cache disabled.', 'redis-cache'), 'updated') : add_settings_error('redis-cache', 'dropin', __('Object cache could not be disabled.', 'redis-cache'), 'error');
                    }
                    ...
                    if ($action === 'update-dropin') {
                        ...
                        $result ? add_settings_error('redis-cache', 'dropin', __('Updated object cache drop-in and enabled Redis object cache.', 'redis-cache'), 'updated') : add_settings_error('redis-cache', 'dropin', __('Object cache drop-in could not be updated.', 'redis-cache'), 'error');
                    }
                }
                ...
                if (count($messages) !== 0) {
                    ...
                    wp_safe_redirect(network_admin_url(add_query_arg('settings-updated', 1, $this->page)));
                    exit();
                }
            }
        }
    ...
    public function ajax_dismiss_notice()
    {
        if (isset($_POST['notice'])) {
            ...
            $notice = sprintf('roc_dismissed_%s', sanitize_key($_POST['notice']));
    ...
    public function maybe_print_comment()
    {
        ...
        if ((defined('WP_CLI') && WP_CLI) || (defined('DOING_CRON') && DOING_CRON) || (defined('DOING_AJAX') && DOING_AJAX) || (defined('REST_REQUEST') && REST_REQUEST) || (defined('JSON_REQUEST') && JSON_REQUEST) || (defined('IFRAME_REQUEST') && IFRAME_REQUEST) || (defined('XMLRPC_REQUEST') && XMLRPC_REQUEST) || (defined('WC_API_REQUEST') && WC_API_REQUEST)) {
            return;
        }
        ...
        if (!isset($wp_object_cache->cache, $wp_object_cache->diagnostics) || !is_array($wp_object_cache->cache)) {
            return;
        }
        ...
        $message = sprintf('Performance optimized by Redis Object Cache. Learn more: %s', 'https://wprediscache.com');
    ...
    {
        return apply_filters('file_mod_allowed', !defined('DISALLOW_FILE_MODS') || !DISALLOW_FILE_MODS, 'object_cache_dropin');
    }
    ...
    public function action_link($action)
    {
        ...
        return wp_nonce_url(network_admin_url(add_query_arg('action', $action, $this->page)), $action);
    }
    ...
    {
        return preg_replace('/password=[^&]+/', 'password=*****', (string) $url);
    }
